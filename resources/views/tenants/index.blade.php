<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <x-button href="{{ route('tenants.create') }}">
    ...
    <div class="py-12">
        <div class="w-full mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <x-card>
                <div class="p-6">
                    <form method="GET" action="{{ route('tenants.index') }}" class="flex flex-wrap gap-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(request()->hasAny(['search']))
                                <x-button href="{{ route('tenants.index') }}" variant="secondary">
                                    Clear
                                </x-button>
                            @endif
                        </div>
            ...
            <x-card>
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">All Tenants ({{ $tenants->total() }})</h3>
                </div>
                                    ...
                                    @foreach($tenants as $tenant)
                                        <tr class="hover:bg-gradient-to-r hover:from-gray-50 hover:to-blue-50 transition-all duration-200">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @if($tenant->tenantProfile && $tenant->tenantProfile->apartment)
                                                    <div class="text-sm font-medium text-gray-900">
                                                        {{ $tenant->tenantProfile->apartment->number }}
                                                    </div>
                                                    <div class="text-sm text-gray-500">
                                                        {{ strtoupper($tenant->tenantProfile->apartment->type) }}
                                                        @if($tenant->tenantProfile->apartment->block)
                                                            - Block {{ $tenant->tenantProfile->apartment->block }}
                                                        @endif
                                                    </div>
                                                @else
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                @if($tenant->tenantProfile && $tenant->tenantProfile->lease_start)
                                                    <div class="font-medium">{{ $tenant->tenantProfile->lease_start->format('M d, Y') }}</div>
                                                    @if($tenant->tenantProfile->lease_end)
                                                        <div class="text-gray-500 text-xs">to {{ $tenant->tenantProfile->lease_end->format('M d, Y') }}</div>
                                                    @endif
                                                @else
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <div class="flex items-center">
                                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">
                                                        {{ $tenant->leases_count ?? 0 }} {{ $tenant->leases_count == 1 ? 'lease' : 'leases' }}
                                                    </span>
                                                    @if($tenant->leases_count > 0)
                                                        <a href="{{ route('tenants.show', $tenant) }}#leases" 
