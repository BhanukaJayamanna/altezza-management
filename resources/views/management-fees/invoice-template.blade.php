...
<head>
    ...
    <title>Management Corporation Invoice - {{ $invoice->invoice_number }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="invoice-container">
        ...
        <div class="header">
            ...
            <div class="company-info">
                ...
                <div class="company-details">
                    <div>(Condominium Plan No. {{ $managementCorp->plan_number ?? '7538' }}, Registration No. {{ $managementCorp->registration_number ?? 'CMA/CCU/2023/PVT/MC/1018' }})</div>
                    <div>{{ $managementCorp->address ?? 'No. 202/1, AVERIWATTA ROAD, HUNUPITIYA, WATTALA' }}</div>
                    <div>E-mail: {{ $managementCorp->email ?? 'propertymanager.altezza@gmail.com' }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="outstanding-section">
            <table class="outstanding-table">
                ...
                <tbody>
                    <tr>
                        <td>1</td>
                        <td>Previous Outstanding as of {{ $previousPeriodEnd ?? '1st JANUARY ' . $invoice->year }}</td>
                        <td class="amount-col">{{ number_format($previousOutstanding ?? 0, 2) }}</td>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>Total Payments as of {{ $paymentsAsOf ?? '25th MARCH ' . $invoice->year }}</td>
                        <td class="amount-col">{{ number_format($totalPayments ?? 0, 2) }}</td>
                    </tr>
                    <tr>
                        <td></td>
                        <td><strong>Total Outstanding</strong></td>
                        <td class="amount-col"><strong>{{ number_format($currentOutstanding ?? 0, 2) }}</strong></td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <table class="charges-table">
            ...
            <tbody>
                <tr>
                    <td>3</td>
                    <td class="description-col">Management Fees for {{ $invoice->billing_period_start->format('M-y') }} to {{ $invoice->billing_period_end->format('M-y') }}</td>
                    <td>SQFT</td>
                    <td>{{ number_format($invoice->area_sqft, 2) }}</td>
                    <td>{{ number_format($invoice->management_fee_ratio, 2) }}</td>
                    <td>3</td>
                    <td class="amount-col">{{ number_format($invoice->quarterly_management_fee, 2) }}</td>
                </tr>
                <tr>
                    <td>4</td>
                    <td class="description-col">Sinking Fund for {{ $invoice->billing_period_start->format('M-y') }} to {{ $invoice->billing_period_end->format('M-y') }}</td>
                    <td>SQFT</td>
                    <td>{{ number_format($invoice->area_sqft, 2) }}</td>
                    <td>{{ number_format($invoice->sinking_fund_ratio, 2) }}</td>
                    <td>3</td>
                    <td class="amount-col">{{ number_format($invoice->quarterly_sinking_fund, 2) }}</td>
                </tr>
                ...
                <tr>
                    ...
                    <td>%</td>
                    <td>2%</td>
                    <td>{{ number_format($invoice->total_amount - $invoice->late_fee, 2) }}</td>
                    <td>0.00</td>
                    <td class="amount-col">{{ number_format($invoice->late_fee, 2) }}</td>
                </tr>
                @else
                <tr>
                    <td>5</td>
                ...
                <tr style="border-top: 2px solid #000;">
                    ...
                    <td class="amount-col" style="font-weight: bold;">{{ number_format($invoice->total_amount, 2) }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="summary-section">
            <table class="summary-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>B/F</td>
                        <td>{{ number_format($currentOutstanding ?? 0, 2) }}</td>
                    </tr>
                    <tr class="total-amount">
                        <td><strong>Total Amount Payable</strong></td>
                        <td><strong>{{ number_format(($invoice->total_amount + ($currentOutstanding ?? 0)), 2) }}</strong></td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="bank-details">
            ...
            <table class="bank-table">
                <tr>
                    <td>A/C Name</td>
                    <td>{{ $bankDetails['account_name'] ?? 'THE MCCP NO. 7538 ALTEZZA APARTMENT' }}</td>
                </tr>
                <tr>
                    <td>A/C No</td>
                    <td>{{ $bankDetails['account_number'] ?? '035010047455' }}</td>
                </tr>
                <tr>
                    <td>A/C Type</td>
                    <td>{{ $bankDetails['account_type'] ?? 'CURRENT ACCOUNT' }}</td>
                </tr>
                <tr>
                    <td>Bank Name</td>
                    <td>{{ $bankDetails['bank_name'] ?? 'Hatton National Bank (7083)' }}</td>
                </tr>
                <tr>
                    <td>Branch Name</td>
                    <td>{{ $bankDetails['branch_name'] ?? 'Wattala (035)' }}</td>
                </tr>
                <tr>
                    <td>Swift Code</td>
                    <td>{{ $bankDetails['swift_code'] ?? 'HBLILKLX' }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="terms-conditions">
            ...
            <ol>
                ...
                <li><strong>Any payment made after 25th MARCH {{ $invoice->year }} might not be reflected in the outstanding given above.</strong></li>
                ...
                <li><strong>Those who have an outstanding balance as of 30 MARCH {{ $invoice->year }}, please be kind enough to settle the same on or before 15th APRIL {{ $invoice->year }} as per the CMA act, legal action will be taken.</strong></li>
            </ol>
        </div>
