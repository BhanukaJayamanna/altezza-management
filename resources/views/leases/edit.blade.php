<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Lease') }} #{{ $lease->lease_number }}
            </h2>
            <div class="space-x-2">
                <a href="{{ route('leases.show', $lease) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700">
                    View Lease
                </a>
    ...
    <div class="py-12">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            ...
            <x-card>
                <form method="POST" action="{{ route('leases.update', $lease) }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="lease_number" class="block text-sm font-medium text-gray-700 mb-2">
                                    Lease Number
                                </label>
                                <input type="text" 
                                       id="lease_number" 
                                ...
                                <select id="owner_id" 
                                    ...
                                    @foreach($owners as $owner)
                                        <option value="{{ $owner->id }}" 
                                                {{ (old('owner_id', $lease->owner_id) == $owner->id) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" 
                                           ...
                                           value="{{ old('maintenance_charge', $lease->maintenance_charge) }}" 
                                           step="0.01" 
                                           min="0"
                    ...
                    <div class="border-t pt-6">
                        ...
                        <div id="lease-terms" class="space-y-3">
                            @if($lease->terms_conditions && count($lease->terms_conditions) > 0)
                                @foreach($lease->terms_conditions as $index => $term)
                                    <div class="flex items-center space-x-3">
                                        <input type="text" 
                                               name="terms_conditions[]" 
                                               value="{{ old('terms_conditions.' . $index, $term) }}"
                    ...
                    <div class="border-t pt-6">
                        ...
                        <p class="text-sm text-gray-600 mb-4">Upload a new contract file or keep the existing one.</p>
                            ...
                            <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm text-blue-800">Current contract: {{ basename($lease->contract_file) }}</span>
                                    </div>
                                    <a href="{{ Storage::url($lease->contract_file) }}" target="_blank" class="text-blue-600 hover:text-blue-800 text-sm">
                                        View
                                    </a>
                                </div>
                            </div>
                        ...
                        <div>
                            <label for="contract_file" class="block text-sm font-medium text-gray-700 mb-2">
                                {{ $lease->contract_file ? 'Replace Contract File' : 'Contract File' }}
                            </label>
                            <input type="file" 
                                   id="contract_file" 
                                   name="contract_file" 
